...
public class LoginService {
    ...
    public SysUser adminLogin(SysUser sysUser) {
        ...
        SysUser dbSysUser = loginMapper.getBySysUser(username, phone, email);
    ...
    public SysUser clientLogin(SysUser sysUser) {
        ...
        String email = sysUser.getEmail();
        String phone = sysUser.getPhone();
        SysUser dbSysUser = loginMapper.getSysUserByPetUsername(username, phone, email);
...
//        检查邮箱是否存在
        if (dbSysUser.getEmail() == null || dbSysUser.getEmail().isEmpty()) {
            throw new CustomException(400, "请先绑定邮箱");
        }
